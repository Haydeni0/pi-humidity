...
def updateGraph(n: int) -> tuple[dict, dict]:
    ...
    for sensor, name, colour in zip(
        ...
        sensor.database.cursor.execute(
            f"""
        SELECT time AS dtime, value AS humidity FROM unnest(
            (SELECT asap_smooth(dtime, humidity, 10)
            FROM test.dht_inside)
            );
            """
        )

        H_traces.append(
            go.Scatter(x=D, y=H, marker_color=colour, line={"dash": "dash"}, name=name)
        )
